...
def test_stackref_order(mocker, stack_params_and_outputs_and_stack):
    ...
    assert (
        result["Resources"]["resTaskDefinition"]["Properties"]["ContainerDefinitions"][
            0
        ]["Environment"][0]["Value"]
        == '{\n  "s3": [{\n    "path": "/${x-forwarded-for}/*",\n    "bucket": "dev-my-test-bucket",\n    "basePath": "",\n    "region": "${AWS::Region}"\n  }]\n}\n'
    )
    assert (
        result["Resources"]["resBackendRole"]["Properties"]["Policies"][0][
            "PolicyDocument"
        ]["Statement"][1]["Resource"]
        == "arn:aws:s3:::dev-my-test-bucket/"
    )
    assert (
        result["Resources"]["resInboxPolicy"]["Properties"]["Bucket"]["Fn::Sub"]
        == "${myBucket.Arn}/*"
    )
...
def test_compound_prorety_values(mocker):
    result = OrderedDict()
    import_parameter_file(
        "n_utils/tests/properties/test-compound-values.properties", result
    )
...
def test_compound_values_for_yaml(mocker):
    ...
    yaml_result = yaml_to_dict(
        "n_utils/tests/templates/test-compound-values.yaml", extra_parameters=result
    )
